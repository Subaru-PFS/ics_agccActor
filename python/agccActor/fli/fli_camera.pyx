...
class Camera:
	...
	def getNextFilename(self, expType):
		...
		return os.path.join(path, 'AGC_%d_%s_%d_%s.fits' % \
...
#		else:
#			raise FliError("Camera not in exposing state")
	...
	def expose(self, dark=False):
		...
		if self.status != READY:
			print "Camera not ready, command aborted"
			return
		self.dark = dark
		if dark:
		...
		self.timestamp = datetime.now().strftime("%Y%m%d%H%M%S%f")[:-5]
