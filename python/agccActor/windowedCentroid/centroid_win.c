...
//struct cand_point *cand_head = NULL;
  ...
  int globalBack=((struct thread_data*)arg)->globalBack;          //data threshold
  ...
  cand_list=getRegions(image,thresh1,thresh2,boxFind,boxCent,n_x,n_y,nmin,nmax,globalBack,imagemask,&np,verbose);
  ...
  while(cand_curr!=NULL)
  {
    ...
    //printf("LL %lf %lf\n",cand_curr->xpeak,cand_curr->ypeak);

    centroidVal=windowedPos(image,cand_curr->x,cand_curr->y, cand_curr->back, boxCent,fwhmx,fwhmy,maxIt,n_x,n_y,verbose);
    ...
    cand_curr->x2=centroidVal[3];
    cand_curr->y2=centroidVal[4];
    cand_curr->xy=centroidVal[5];
    cand_curr->flux=centroidVal[6];
    cand_curr->npix=centroidVal[7];
...
struct centroids *centroid(int *image, int n_x, int n_y, int thresh1, int thresh2, double fwhmx, double fwhmy,int boxFind, int boxCent,int *np, int nmin, int nmax,int maxIt, int globalBack, int verbose)
{
  ...
  for (ii=0;ii<XSPLIT;ii++)
      ...
      for (jj=0;jj<YSPLIT;jj++)
	{
	  ...
	  thread_data_array[ind].globalBack=globalBack;
